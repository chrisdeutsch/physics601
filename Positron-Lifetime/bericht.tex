...
\end{align*}
berechnet \cite{add_infos} und der zugehörige Fehler durch \textsc{Gauß}sche Fehlerfortpflanzung gebildet.
...
Um die Lebenszeit des ungebundenen Positrons~$\tau_f$ zu bestimmen, kann eine Anpassung an die aus der Theorie erwartete mittlere Lebenszeit 
\begin{align*}
...
Vergleicht man den Parameter $b$ mit Gleichung \eqref{eq:mean_lifetime} so kann die Leerstellenenthalpie durch
\begin{align*}
	H_t = k_\mathrm{B} \cdot b
\end{align*}
mit $k_\mathrm{B} \approx \num{8.617} \cdot 10^{-5} \, \si{\eV\per\kelvin}$ \cite{codata} berechnet werden zu
\begin{align*}
