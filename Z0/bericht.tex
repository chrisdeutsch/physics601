...
\begin{figure}[h]
	\centering
	\includegraphics[width=.8\textwidth]{./data/tag1/ee_pics/cropped/ee_02}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=.8\textwidth]{./data/tag1/mm_pics/cropped/mm_02}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=.8\textwidth]{./data/tag1/tt_pics/cropped/tt_05}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=.8\textwidth]{./data/tag1/qq_pics/cropped/qq_04}
...
\korr{Final cut counts \& precut counts}
The error of the number of events after applying a cut can be estimated by the variance of the underlying binomial distribution
\begin{align*}
	\Delta n_i = \sqrt{N \cdot \left[ \frac{n_i}{N} - \left( \frac{n_i}{N} \right)^2 \right]} \quad \text{with}\quad i \in \{ \mathrm{e}, \mathrm{\mu}, \mathrm{\tau}, \mathrm{q} \}
\end{align*}
with the number of events $n_i$ after the $i$-cut and the total number of events~$N$.
...
\korr{Number of s-channel events}
\begin{align*}
	n_\mathrm{e,s-channel} = n_\mathrm{e} \cdot \mathrm{precut} \cdot \mathrm{anglecut}
\end{align*}


\subsubsection{Efficiency Matrix}
By setting cuts on the data one loses a fraction of the simulated events and in addition to this misidentifications of events using the cuts can occur.
Knowing that each datasets was simulated with \num{100000} events the efficiency of the event selection using cuts can be evaluated.
...
To calculate the efficiency matrix according to equation \eqref{eq:efficiency_matrix_elements} one has to note that $N_\mathrm{e} \neq 100000$ but rather the number of events in the s-channel \korr{as determined in s-t-channel chapter}.
Using the number of events after the different cuts in table \korr{REF} and their errors according to the underlying binomial distribution, the matrix elements of the efficiency matrix can be calculated
\begin{align*}
	&E = \begin{pmatrix}
	...
	\end{pmatrix}
\end{align*}
and the error matrix using Gaussian Error Propagation
\begin{align*}
...
\korr{Interpretation: almost diagonal, efficiencies etc.}
...
For this a sample of $\num{100000}$ different efficiency matrices was generated according to the standard deviations as given in the error matrix.
...
\begin{table}
	\begin{subtable}{1.0\textwidth}
		...
		\subcaption{Counts for cross section measurement. Error estimated from binomial statistics.}
	...
	\begin{subtable}{1.0\textwidth}
		...
		\subcaption{Corrected counts for cross section measurement}
...
\begin{align*}
	...
	&f(x=\mu; A, \sigma) = \frac{A}{\pi \sigma} \\
	&\Gamma = 2 \sigma \\
	&\sigma^\mathrm{peak} = \frac{A}{\pi \sigma}
\end{align*}
...
\begin{align*}
	N_\nu = \frac{\Gamma_\mathrm{tot} - \Gamma_\mathrm{e} - \Gamma_\mathrm{\mu} - \Gamma_\mathrm{\tau} - \Gamma_\mathrm{hadr}}{\Gamma_\nu}
\end{align*}
