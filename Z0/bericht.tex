...
Those are defined by the angle $\theta$ \korr{Großes Theta kleine Theta?} between incoming positron and outgoing positively charged particle.
...
\korr{Momentum of the electrons (small mass -- equal energy)? Curvature in magnetic field (barely any because electrons carry most of the momentum but can be used for momentum measurement)? }
A decay into electrons can easily be identified by the energy deposited in the electromagnetic calorimeters.
The final state particles are flying approximately in opposite direction and are ideally completely stopped in the electromagnetic calorimeter.\korr{How does this work? Electromagnetic shower.}
A track for every particle can be observed in the tracking chamber.
Additional hits in the electromagnetic calorimeters probably occur due to bremsstrahlung photons which leave no track in the inner detector\korr{Was bedeutet das für die Variablen? -- Die CM energie kann nicht im SumP beobachtet werden weil Photon ungeladen jedoch im ecal}. \korr{Woher kommen mehr als 2 tracks?}
\korr{Interpretation der Tabellen muss aufjedenfall gemacht werden. D.h. typische response -- electronen wenig tracks fast die gesamte energie (SumP = Ecal) im Calo deponiert. Geladene tracks tragen die gesamte energie, hcal is nichts bis kaum was}
\korr{Außerdem fehlen verweise auf das eventdisplay und die tabelle}
\begin{sidewaystable}
	\centering
...
Muons are detected in the muon chambers which form the outermost part of the detector.\korr{Was ist das? Ich würde den nachfolgenden satz mit dem hier tauschen. -- Muonen interagieren kaum weil soundso daher in muonendetektoren (trackern) im außenbereich ist man sich sicher dass es muonen sind}
Since all other particles (except the non-detectable neutrinos) are stopped before reaching the muon chamber a detected particle there can be identified as a muon\korr{Warum tun die das nicht? Viel schwerer und damit geringere wahrscheinlichkeit schauer auszulösen da bremsstrahlung stark unterdrückt (wie ist die proportionalität?)}. 
