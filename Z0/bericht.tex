...
With the help of a data analysis software the mass, decay width and partial cross sections of the $\mathrm{Z}$-boson shall be determined and a parameters of the Standard Model, the Weinberg angle, shall be calculated.
...
\end{align*}
The values for the weak charge $Q_f$, the third component of the weak isospin $I_3^f$ and the Weinberg angle $\theta_\mathrm{W}$ are given in table \korr{REF}\korr{Tabelle mit weak charges, ladungen und kopplungsfaktoren für die verschiedenen teilchen}\korr{Thomson 423}\\
...
After analyzing the distributions of measured parameters cuts have been defined to distinguish between the occurring decay channels.
This is needed in order to develop a machinable selection of events by decay type.
Using a Monte-Carlo simulated test sample with all types of events the applied cuts could be verified by comparing the decay channel found by the computer and the one found by eye in the event display.
