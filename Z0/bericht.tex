...
\subsection{Problems}
In the following the questions that have been solved before performing the experiment are presented.
\label{sec:problems}
...
This event can be interpreted as a $t$-channel reaction involving two photons where additionally a fermion-antifermion pair is produced.
Those can be observed while the electron and positron in the final state have a large boost along the beam axis and hence are not detected.
Since they carry a lot of the energy the detected charged particles carry a noticeably small momentum.
Based on the signature, none of the classifications would be applicable for this event.
\begin{sidewaystable}
	\centering
...
\subsection{Misinterpreted Event in the Test Sample}
\begin{figure}[h]
	\centering
	\begin{subfigure}{\textwidth}
		\centering
...
\begin{figure}[h]
	...
	\begin{subfigure}{\textwidth}
		...
		\subcaption{Side view}
	\end{subfigure}
	\caption{Misinterpreted event in the test sample. It happens to be a two-photon reaction for which no classification is applicable. Additionally, an electron positron pair must be produced which can not be detected since it is strongly boosted in direction along the beam axis.}
	\label{fig:2photon}
\end{figure}
