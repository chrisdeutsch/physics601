...
\begin{frame}{$\mathrm{Z}^0$ Boson}
	\begin{itemize}
		\item Mass $M_\mathrm{Z} = \SI{91.1876 +- 0.0021}{GeV}$ \cite{pdg}
		\item Decay width $\Gamma_\mathrm{Z} = \SI{2.4952 +- 0.0023}{GeV}$ \cite{pdg}
	...
	\begin{columns}
		\column[c]{0.5\textwidth}
			\begin{itemize}
				...
				\item<3-> remove most $t$-channel events by cutting \framebox[1.1\width]{$\cos\theta < 0.5$}
