...
\korr{Erklären was in welchem Fenster in ATLANTIS angezeigt wird.}
...
Das Magnetfeld im inneren Detektor, welches in Richtung der Strahlröhre zeigt, zwingt die Elektronen dort auf eine Kreisbogensegment und wird zur Rekonstruktion von Ladung und transversalem Impuls genutzt.
Im Fall von Abbildung \ref{fig:electron-curvature} kann an der Krümmung abgelesen werden, dass es sich um ein negativ geladenes Elektron handelt (das Magnetfeld zeigt in der $x$-$y$-Ansicht aus der Bildebene).
Schließlich bildet das Elektron im elektromagnetischen Kalorimeter einen Kaskade aus Photonen und Elektron-Positron-Paaren aufgrund von Bremsstrahlungs- und Paarbildungsprozessen.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{./data/atlantis/singlepart_events_new/muon/single_track2.png}
	\caption{Single Myon}
	\label{fig:myon-singletrack}
\end{figure}

In Abbildung \ref{fig:myon-singletrack} ist die typische Detektorantwort auf ein Myon zu sehen.
Da es sich bei Myonen um geladene Teilchen handelt, hinterlassen diese eine Spur im inneren Detektor und fliegen durch das dort herrschende Magnetfeld auf einer gekrümmten Bahn.
Sie passieren die beiden Kalorimeter beinahe ungehindert, deponieren jedoch aufgrund von \korr{warum?} eine geringe Energiemenge im hadronischen Kalorimeter.
Im Myonsystem, das mit einer eigenen Spurrekonstruktion und einem toroidalen Magnetfeld ausgestattet ist, kann ebenfalls eine Spur dargestellt werden, die aufgrund eines anderen Magnetfelds eine andere Krümmung in der $\rho$z-Ebene aufweist als im inneren Detektor.
Da andere hadronische oder leptonische Teilchen im elektromagnetischen oder hadronischen Kalorimeter gestoppt worden wären und Neutrinos keine Spuren in einem Tracker hinterlassen, ist einer Teilchensignatur wie hier dargestellt zweifelsfrei ein Myon zuzuordnen. 
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{./data/atlantis/singlepart_events_new/photons/single_photon_2.png}
	\caption{Single Photon}
	\label{fig:photon}
\end{figure}
Wie in Abbildung \ref{fig:photon} zu sehen, hinterlassen Photonen als ungeladene Teilchen keine rekonstruierbaren Spuren im Detektor.
Da sie jedoch elektromagnetisch interagieren erzeugen sie einen Schauer im elektromagnetischen Kalorimeter, wo sie ihre gesamte Energie abgeben und somit gestoppt werden.
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{./data/atlantis/singlepart_events_new/photons/conversion.png}
	\caption{Konversion}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{./data/atlantis/singlepart_events_new/tau/muon.png}
	\caption{Event mit Myon}
	\label{fig:tau-muon}
\end{figure}
Das Ereignis in Abbildung \ref{fig:tau-muon} des $\tau$-Lerndatensatzes zeigt den Zerfall eines $\tau$-Leptons in ein Myon.
Aufgrund der kurzen Lebensdauer zerfällt das $\tau$-Lepton nach sehr kurzer Flugstrecke, so dass die beobachtbare rekonstruierbare Trajektorie auf Spurpunkten des Myons basiert.
Prinzipiell ist auch ein Zerfall in ein Elektron möglich, dieses würde einen Schauer im elektromagnetischen Kalorimeter verursachen und nicht wie bei dem hier betrachteten Ereignis im Myonsystem registriert werden.
In beiden Fällen entstehen zusätzlich zu dem geladenen Lepton auch ein $\tau$-Neutrino und das jeweilige Antineutrino des Elektrons bzw. Myons.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=.8\textwidth]{./data/atlantis/singlepart_events_new/tau/single_pion.png}
	\caption{Einzelnes Pion}
	\label{fig:tau-pion}
\end{figure}
Ein Beispiel für einen hadronischen $\tau$-Zerfall liefert Abbildung \ref{fig:tau-pion}.
Unter Aussendung eines $\tau$-Neutrinos entstehen in hadronischen Zerfällen oftmals Pionen.
Durch die sichtbare Trajektorie des Teilchens muss in dem hier betrachteten Zerfall ein geladenes Pion entstanden sein, welches im hadronischen Kalorimeter gestoppt wird und dort Energieeinträge hervorruft.
Es ist ebenfalls erneut die fehlende transversale Energie aufgrund des $\tau$-Neutrinos zu sehen.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{./data/atlantis/singlepart_events_new/jets/muon.png}
...
Die geladenen Pionen haben eine Zerfallslänge von in der Ordnung von $c \tau = \SI{7.8}{\meter}$~\cite{pdg} und können dadurch direkt in den hadronischen Kalorimetern beobachtet werden.
