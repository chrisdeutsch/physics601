...
\begin{figure}[htbp]
	...
	\begin{overpic}[width=\textwidth,tics=10]{./data/root/wmass/exercise3/ATLAS_cut}
		\put (25,1) {Transversalimpuls des W-Bosons $p_\mathrm{T}$ / \si{GeV}}
		\put (1,29) {\rotatebox{90}{Ereignisse}}
	\end{overpic}
...
A priori ist nicht klar, ob der angepasste Halbhöhepunktes~$h$ der Polstelle in Gleichung \eqref{eq:pt_spektrum} entspricht.
...
Zur Bestimmung der Eichkurve wird die simulierte W-Masse~$M_\mathrm{MC}$ gegen die angepasste Position des Halbhöhepunkts~$h$ aufgetragen und eine Gerade an die Daten angepasst.
Um bei der Anpassung der Geraden die Fehler in der Halbhöhepunkt-Messung korrekt zu beachten, wird diese durch eine orthogonale Regression \cite{odr} durchgeführt.
...
\subsection{W-Masse}
In Abschnitt \korr{REF} wurde die Position des Halbhöhepunktes~$h$ für die simulierten und kalibrierten ATLAS-Daten zu
\begin{align}
	h = \SI{42.10 +- 0.07}{\GeV}
	\label{eq:messwert_w_h}
\end{align}
bestimmt.
Um die W-Masse mithilfe der zuvor angepassten Eichkurve zu bestimmen, ist es nötig die gemeinsame Kovarianzmatrix für die Kurvenparameter $m, b$ und des Halbhöhepunktes~$h$ zu bestimmen.
...
\begin{align*}
	...
	\end{pmatrix} \, \text{.}
\end{align*}
Dadurch erhält man nach der Auswertung der Eichkurve durch Anwendung von \textsc{Gauß}scher Fehlerfortpflanzung gemäß \korr{CITE}
\begin{align*}
...
Setzt man die Kurvenparameter und die Position des Halbhöhepunktes sowie deren Fehler ein, so erhält man für die W-Masse
\begin{align*}
