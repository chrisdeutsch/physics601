...
Bei den betrachteten Ereignissen können mithilfe der Ereignisanzeige Atlantis die Myonenparameter (Impuls~$p$, transversaler Impuls~$p_\mathrm{T}$, Pseudorapidität~$\eta$, \dots) der Myonen jeweils im inneren Detektor und im Myonensystem abgefragt werden.

Um den Energieverlust der Myonen im Kalorimeter zu bestimmen, ist es nötig den Impuls~$p$ ebendieser zu bestimmen.
Da Atlantis auf diesen Parameter jedoch keinen Fehler angibt, wurde stattdessen der transversale Impuls~$p_\mathrm{T}$ und die Pseudorapidität~$\eta$ bestimmt, um die Fehlerfortpflanzung per Hand durchzuführen.
Nach der Definition in Gleichung \eqref{eq:pseudorapidity} und $p = p_\mathrm{T} / \sin\theta$ kann der Impuls gemäß
\begin{align*}
...
In Abbildung \ref{fig:muon_eloss_eta} wurde der Energieverlust gegen den Betrag der Pseudorapidität aufgetragen und zeigt, dass keine starke Abhängigkeit festgestellt werden kann.
...
Da sich diese jedoch in der Größenordnung von mehreren \SI{100}{\GeV} \cite{pdg} liegt und der größte vermessene Impuls lediglich \SI{200}{\GeV} beträgt, sollte diese Abhängigkeit nicht in Erscheinung treten.
...
Im Wesentlichen liegt dies daran, dass Energieverlust von Teilchen in Materie ein statistischer Prozess ist und man lediglich eine Distribution für diesen angeben kann (Laundau-Distribution).
...
\end{align*}
mit einem reduzierten $\chi$-Quadrat $\chi_\mathrm{red.}^2 = \num{3.11}$ und wurde ebenfalls in den Abbildungen \ref{fig:muon_eloss} aufgetragen.
...
Zur Bestimmung der invarianten Elektron-Positron-Masse aus drei~$Z^0 \rightarrow e^+ + e^-$ Zerfällen wurden mit Atlantis die Energien und Winkelinformationen der jeweiligen Teilchenspuren ausgelesen.
Hierzu wurde die Darstellung der Ereignisse in der~$\phi\eta$-Ebene genutzt, um zweifelsfrei die Energieeinträge im Kalorimeter den Trajektorien von Elektron und Positron zuzuordnen.
...
\begin{align}
	m_\mathrm{inv}&=\sqrt{m^2_{\mathrm{e}^-} + m^2_{\mathrm{e}^+} + 2 E_{\mathrm{e}^-} E_{\mathrm{e}^+} - 2\vec{p}_{\mathrm{e}^-}\vec{p}_{\mathrm{e}^+}} \\
	m_\mathrm{inv}&=\sqrt{m^2_{\mathrm{e}^-} + m^2_{\mathrm{e}^+} + 2 E_{\mathrm{e}^-} E_{\mathrm{e}^+} - 2|\vec{p}_{\mathrm{e}^-}||\vec{p}_{\mathrm{e}^+}|\cos(\theta)}\,\text{,}
\end{align}
wobei $\theta$ den Winkel zwischen den beiden Teilchen angibt.
Da nur der Betrag des Impulses eines Teilchens gemessen werden kann, werden die Winkelinformationen~$\phi_0$ und~$\eta$ benötigt, um auf~$\theta$ zu schließen.
