% !TeX document-id = {1a59407d-6137-47a3-bc51-2f068fe21e02}
...
Die Ergebnisse zeigen eine Abweichung von der erwarteten $\mathrm{Z}$-Masse \cite{pdg}
\begin{align*}
...
Eine mögliche Quelle für diesen Unterschied ist der Energieverlust der Elektronen aufgrund von Bremsstrahlung in der Strahlröhre und dem Silizium der Halbleiterdetektoren direkt um den Interaktionsvertex.
...
Die Kalibration wird durch C-Code in der Datei \texttt{EleCalib.c} implementiert, die in Codeausschnitt \ref{list:elecalib} beispielhaft (für eine Iteration des $\eta$ Parameters) angegeben ist.
\begin{listing}[!h]
\begin{minted}[xleftmargin=0em, xrightmargin=0em, tabsize=4, frame=lines, framesep=2mm, style=xcode]{c}
#include "math.h"
#include "TMath.h"
...
\end{minted}
\caption{Beispiel für eine Kalibrationsdatei}
\label{list:elecalib}
\end{listing}
...
Im Anschluss wurde die gesamte Verteilung so kalibriert, dass der Schwerpunkt des Peaks mit der tatsächlichen Z-Masse zusammenfällt.
Die endgültige Kalibration ist in Abbildung \ref{fig:final_calibration} gezeigt.
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{./data/root/calibration/new/calibrated.pdf}
