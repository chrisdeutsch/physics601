...
Es eignet sich der Übergang $F=3 \rightarrow F^\prime=4$, da dieser beinahe abgeschlossen ist, d.h. dass das Atom durch Abregung vom Zustand $F^\prime=4$ beinahe ausschließlich in den Zustand $F=3$ übergeht.
Dieser Übergang entspricht einer Wellenlänge von \SI{780.25}{nm}, so dass ein entsprechender Laser zur resonanten Anregung gewählt wird.
...
Hierzu werden zwei Diodenlaser in Littrow-Konfiguration genutzt, d.h. das emittierte Laserlicht wird kollimiert und trifft auf ein Reflexionsgitter, welches so in den Strahlengang gebracht wird, dass die Beugung der~-1.~Ordnung zurückreflektiert wird und somit einen externen Resonator bildet, während das Licht, welches in~0.~Ordnung gebeugt wird, ausgekoppelt wird.
...
Diese Bewegung ist für die Dopplerverschiebung verantwortlich und erklärt die Abflachung des Signals im mittleren Frequenzbereich.
Das Maximum, auch Lamb-Dip genannt, ergibt sich durch Atome, die sich in einer Ebene senkrecht zu den Laserstrahlen bewegen.
Die Dopplerverschiebung verschwindet hier, so dass der Pumpstrahl und Teststrahl mit der gleichen Frequenz im Ruhesystems des Atoms strahlen und der Übergang somit gesättigt wird, weswegen der Teststrahl keinen Intensitätsverlust erfährt.
Die natürliche Linienbreite kann auf diese Weise beobachtet werden.

In der Polarisationsspektroskopie wird der Pumpstrahl zirkularpolarisiert, so dass die $m_\mathrm{J}=\pm1$ Niveaus gesättigt werden.
...
Dieser Winkel ist abhängig von der Änderung des Brechungsindex, die über die Kramers-Kronig-Relation mit dem Absorptionsprofil zusammenhängt \korr{scheiß Relation irgendwie erläutern}.
...
Durch Subtraktion der beiden Spektren kann ein Signal generiert werden, mit dem eine Rückkopplungsschleife betrieben wird, die das Piezoelement am Reflexionsgitter ansteuert, um die Frequenz des Lasers anzupassen.
...
Der in diesem Versuch bestimmte Wert fällt ebenfalls in diesen Bereich, sollte jedoch nur als grobe Abschätzung angesehen werden, da der genaue Druck in der Vakuumkammer nicht bekannt war.
